...
function createData(name, id, phone, department, gNo , nidImage, key) {
  return { name, id, phone, department, gNo , nidImage, key };
}
          ...
          <IconButton aria-label="SaveIcon">
            <SaveIcon onClick={ () => {
			  ...
			  selectedStudents.forEach(std => {
				db.collection('students').doc(std.key).update(
